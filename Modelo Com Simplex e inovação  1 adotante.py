...
for p in range(100):
    ...
    # Iniciando loop de tempo para intermediação
    for t in range(T):
        ofertas = [0]*culturas
        falencias = 0
    ...
    for i in range(n):
        ...
        G.node[i]['renda']=0
        G.node[i]['adotante']=0
    # Iniciando loop de tempo para escolha aleatória
    la=random.sample(list(range(0, n)),inovadores)
    for t in range(T):
        ofertas=[0]*culturas
        falencias=0
...
plt.plot(lad_i, label='Intermediação')
plt.plot(lad_a, label='Escolha Aleatória')
plt.legend(loc='lower left')
plt.xlabel('Tempo')
plt.ylabel('Adotantes')
plt.title("Média de adoção com 1 adotante")
plt.show();
